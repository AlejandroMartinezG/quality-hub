...
import { ShieldCheck, ClipboardCheck, AlertTriangle, Users, ChevronRight } from 'lucide-react'
import { formatFecha } from "@/lib/utils"
import { COLOR_RESULTADO, tasaDiscrepancia, type ResultadoLote } from "@/lib/auditoria-utils"

type LoteResumen = { id: string; resultado: ResultadoLote; nombre_preparador: string | null }
type Auditoria = {
    id: string
    sucursal: string
...
export default function AuditoriasPanel({ auditorias = [] }: Props) {
    ...
    const { kpis, enProceso, cerradas, confiabilidad } = useMemo(() => {
        ...
        const confiabilidad = Array.from(acc.values())
            .map(g => ({ ...g, ...tasaDiscrepancia(g.lotes) }))
            .filter(g => g.n > 0)
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Kpi
                    ...
                    sub={kpis.confiable ? `sobre ${kpis.nRojo} lotes` : `solo ${kpis.nRojo} lotes — aún no es representativo`}
                    icon={AlertTriangle}
                ...
                <CardHeader className="pb-3">
                    <div className="flex flex-wrap items-center gap-3">
                        <div>
                            ...
                            <CardDescription>
                                Porcentaje de lotes donde la medición de Calidad cambió el veredicto
                            </CardDescription>
                        </div>
                                ...
                                <tbody>
                                    {confiabilidad.map((g, i) => (
                                        <tr key={`${g.sucursal}-${g.nombre}-${i}`} className="border-b border-slate-100 dark:border-slate-800 last:border-0">
                                            <td className="px-2 py-2.5 font-medium text-slate-800 dark:text-slate-200">{g.nombre}</td>
                                            {vista === 'preparador' && (
                                            ...
                                            <td className={`px-2 py-2.5 text-right font-bold tabular-nums ${!g.confiable ? 'text-slate-400' : g.pct > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                                                {g.pct.toFixed(0)}%
                                                {!g.confiable && <span className="ml-1 text-[10px] font-normal">(n bajo)</span>}
                                            </td>
                                            <td className="px-2 py-2.5">
                                                <div className="h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full ${!g.confiable ? 'bg-slate-300' : g.pct > 0 ? 'bg-red-500' : 'bg-emerald-500'}`}
