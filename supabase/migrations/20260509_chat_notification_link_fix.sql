-- Migration: Fix CHAT_CALIDAD notification link to include ?chat=<measurement_id>
-- and clean up old COMENTARIO_NUEVO notifications that pointed to the NCR panel
-- for NOTA-type NCR records (measurement chat records).

-- ── 1. Update handle_ncr_notifications to use ?chat= link ────────────────────
            ...
            LOOP
                ...
                BEGIN
                    ...
                    VALUES (
                        ...
                        v_notif_title,
                        v_notif_msg,
                        '/calidad?chat=' || (v_ncr_json->>'measurement_id'),
                        jsonb_build_object(
...
-- ── 2. Fix existing CHAT_CALIDAD notifications that have link = '/calidad' ────
-- Update them to include ?chat=<measurement_id> so they auto-open the right chat.
UPDATE public.notifications
SET link = '/calidad?chat=' || (metadata->>'measurement_id')
WHERE type = 'CHAT_CALIDAD'
  AND link = '/calidad'
  AND metadata->>'measurement_id' IS NOT NULL;


-- ── 3. Delete old COMENTARIO_NUEVO notifications that pointed to the NCR panel
-- for NOTA-type chat records (before this migration corrected the trigger).
DELETE FROM public.notifications
WHERE type = 'COMENTARIO_NUEVO'
  AND metadata->>'ncr_id' IN (
      SELECT id::text FROM public.quality_ncr WHERE status::text = 'NOTA'
  );
